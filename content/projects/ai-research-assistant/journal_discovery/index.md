...
Citation-based snowball sampling is the most promising unbuilt idea: every converted paper's own bibliography is sitting right there in its transcribed text, and OpenAlex already exposes a structured "cited by" graph that needs no text-parsing at all to walk forward from a paper already in the corpus — a more organic way to grow it than author or topic search alone. CORE.ac.uk and Columbia's own Academic Commons repository are two more free, real APIs worth adding as open-access tiers ahead of EZProxy; a semi-automated login through a genuine browser (not a spoofed one) remains the honest next step if manual-download volume ever becomes the real bottleneck. Part of the same **[Source Indexer](/projects/academic-hub/source_indexer/)** / **[RAG Analysis](/projects/academic-hub/rag_analysis/)** ecosystem the rest of **AI Research Assistant** builds on, feeding **[Journal Article Transcription](/projects/ai-research-assistant/journal_article_transcription/)** directly.
