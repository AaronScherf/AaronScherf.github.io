...
A few examples from recent sessions: consolidating the site from multiple pages into a single scrolling homepage with anchor-based navigation, adding new project entries — like [Marker PDF Conversion](/projects/marker_conversion/) and [RAG Analysis](/projects/rag_analysis/) — for tools I'm building as part of a larger effort I'm calling **Academic Hub**, and quietly retiring the template's placeholder content by marking it `draft: true` rather than deleting it outright — so it's still there if I ever want it back.
